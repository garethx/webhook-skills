# Supabase Webhooks - Express Example
...
- Both routes use `express.raw()`. The Auth Hook signature covers the **exact
  raw body bytes** — parsing and re-serialising the JSON breaks verification.
- Auth Hooks are **request/response**: the auth flow blocks on your reply and the
  ...
  **5-second budget including up to three retries** (on 429/503) at a two-second
  backoff, so keep the handler fast.
