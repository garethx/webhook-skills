# Paystack Webhooks - Express Example

Minimal example of receiving Paystack webhooks with signature verification.
Paystack's SDKs have no verification helper, so the signature is verified
**manually** with Node's `crypto` (HMAC-SHA512, hex, timing-safe compare).
...
Run the unit/integration tests (they generate real HMAC-SHA512 signatures):
...
- An invalid or missing signature returns **400**; verified events return
  **200** immediately (so processing happens after acknowledging — Paystack times
  out at 30s and retries non-200 responses).
