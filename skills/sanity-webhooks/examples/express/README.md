# Sanity Webhooks - Express Example

Minimal example of receiving Sanity GROQ-powered webhooks with signature
verification using the official [`@sanity/webhook`](https://github.com/sanity-io/webhook-toolkit) SDK.
...
3. Add your Sanity webhook signing secret to `.env` as `SANITY_WEBHOOK_SECRET`.
...
- The route uses `express.text({ type: 'application/json' })` to keep the **raw**
  body — required because the signature is an HMAC over the raw bytes.
