...
Handlers dispatch on the envelope `subscriptionType` (currently only
`TRACKING`). The subscribable event filter exposes a single value,
`ALL_UPDATES`, so USPS sends a notification for **every** tracking update. Within
the tracking payload, the `status` typically falls into these milestones:
