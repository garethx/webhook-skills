# Pipedrive Webhooks - Next.js Example
...
> the webhook. The route handler verifies those credentials with a timing-safe
> comparison.
...
The handler lives at `app/webhooks/pipedrive/route.ts` and serves
`POST /webhooks/pipedrive` on http://localhost:3000.
...
call the route handler directly, asserting 401 (bad/missing credentials), 400
(invalid payload), and 200 (valid delivery).
