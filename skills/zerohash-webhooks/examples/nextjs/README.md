# Zero Hash Webhooks - Next.js Example

Minimal example of receiving Zero Hash webhooks with signature verification
using the Next.js App Router. Zero Hash has no webhook SDK, so signatures are
verified **manually** with Node's `crypto` module.
...
3. Add your Zero Hash **HMAC shared secret**:
   ```bash
...
`POST http://localhost:3000/webhooks/zerohash`.
