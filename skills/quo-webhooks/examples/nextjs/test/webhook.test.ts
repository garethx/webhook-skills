...
// SCHEME B secret. Bare base64, no prefix — what "Reveal signing secret" shows
// in the Quo app. Quo's documented example key decodes to this ASCII string.
...
type RouteModule = typeof import('../app/webhooks/quo/route');
let route: RouteModule;

beforeAll(async () => {
  // Import after the env vars are set — the module reads QUO_MAX_AGE_SECONDS
  // at load time.
  route = await import('../app/webhooks/quo/route');
});
...
// Legacy envelope (apiVersion v2): data.object, plus top-level object: "event".
const LEGACY_EVENT = {
...
function signCurrent(
  body: string,
  { id = 'msg_2abc', timestamp, key = TEST_WEBHOOK_KEY }: {
    id?: string;
    timestamp?: number;
    key?: string;
  } = {}
...
function signLegacy(
  body: string,
  { timestamp, secret = TEST_LEGACY_SECRET }: { timestamp?: number; secret?: string } = {}
): string {
...
function makeRequest(body: string, headers: Record<string, string>) {
  // NextRequest accepts a standard Request; the route only reads text() and headers.
  const { NextRequest } = require('next/server');
  return new NextRequest('https://example.com/webhooks/quo', {
    method: 'POST',
...
describe('Scheme A — current API (webhook-signature)', () => {
  it('accepts a correctly signed delivery', async () => {
    const res = await route.POST(makeRequest(CURRENT_BODY, signCurrent(CURRENT_BODY)));
  ...
  it('rejects a signature made with the wrong key', async () => {
    ...
    const res = await route.POST(makeRequest(CURRENT_BODY, headers));
  ...
  it('rejects a tampered body', async () => {
    ...
    const tampered = JSON.stringify({ ...CURRENT_EVENT, type: 'call.completed' });
    const res = await route.POST(makeRequest(tampered, headers));
    expect(res.status).toBe(400);
  });
  ...
  it('rejects a stale timestamp (replay)', async () => {
    ...
    const res = await route.POST(
      makeRequest(CURRENT_BODY, signCurrent(CURRENT_BODY, { timestamp: stale }))
    );
    expect(res.status).toBe(400);
  });
  ...
  it('rejects a delivery with no signature headers at all', async () => {
    ...
    const res = await route.POST(makeRequest(CURRENT_BODY, {}));
    expect(res.status).toBe(400);
    expect((await res.json()).error).toBe('Missing signature headers');
  });
  ...
  it('rejects a key whose whsec_ prefix was not stripped before decoding', () => {
    ...
    const headers = signCurrent(CURRENT_BODY);
    const wrong = crypto
      .createHmac('sha256', TEST_WEBHOOK_KEY)
  ...
  it('fails closed when no key is configured', () => {
    expect(
      route.verifyQuoSignature(CURRENT_BODY, signCurrent(CURRENT_BODY), undefined)
    ).toBe(false);
  });
});
...
describe('Scheme B — legacy (openphone-signature)', () => {
  it('accepts a correctly signed legacy delivery', async () => {
    const res = await route.POST(
      makeRequest(LEGACY_BODY, { 'openphone-signature': signLegacy(LEGACY_BODY) })
    );
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ received: true });
  });
  ...
  it('parses the documented header shape: hmac;1;<ms>;<base64>', () => {
    const [scheme, version, timestamp, signature] = signLegacy(LEGACY_BODY).split(';');
    expect(scheme).toBe('hmac');
    expect(version).toBe('1');
    expect(timestamp).toHaveLength(13); // UNIX MILLISECONDS
  ...
  it('rejects a signature made with the wrong secret', async () => {
    ...
    const res = await route.POST(
      makeRequest(LEGACY_BODY, {
        'openphone-signature': signLegacy(LEGACY_BODY, { secret: wrong }),
      })
    );
    expect(res.status).toBe(400);
    expect((await res.json()).error).toBe('Invalid signature');
  ...
  it('treats a 13-digit timestamp as milliseconds, not seconds', () => {
    ...
    expect(
      route.verifyQuoLegacySignature(
        LEGACY_BODY,
  ...
  it('accepts when ANY comma-separated signature matches', () => {
    ...
    expect(
      route.verifyQuoLegacySignature(LEGACY_BODY, `${other},${good}`, TEST_LEGACY_SECRET)
    ).toBe(true);
  });

  it('rejects a malformed header that is not four semicolon fields', () => {
    expect(route.verifyQuoLegacySignature(LEGACY_BODY, 'hmac;1;123', TEST_LEGACY_SECRET)).toBe(
      false
    );
    expect(route.verifyQuoLegacySignature(LEGACY_BODY, 'garbage', TEST_LEGACY_SECRET)).toBe(false);
  });

  it('rejects a scheme or version other than hmac;1', () => {
    const [, , timestamp, signature] = signLegacy(LEGACY_BODY).split(';');
    expect(
      route.verifyQuoLegacySignature(
        LEGACY_BODY,
  ...
  it('rejects an undecoded signing secret', () => {
    ...
    const [, , timestamp] = signLegacy(LEGACY_BODY).split(';');
    const wrong = crypto
  ...
  it('fails closed when no legacy secret is configured', () => {
    expect(
      route.verifyQuoLegacySignature(LEGACY_BODY, signLegacy(LEGACY_BODY), undefined)
    ).toBe(false);
  });
});
...
describe('raw body', () => {
  ...
  // JSON. Verification is over the RAW bytes, so a pretty-printed body with the
  // same semantic content must NOT verify against a compact signature.
  const PRETTY_BODY = JSON.stringify(LEGACY_EVENT, null, 2);

  it('rejects a re-serialized body that differs byte-for-byte', () => {
    expect(
      route.verifyQuoLegacySignature(PRETTY_BODY, signLegacy(LEGACY_BODY), TEST_LEGACY_SECRET)
    ).toBe(false);
  });

  it('verifies a pretty-printed body when it is what was actually signed', () => {
    expect(
      route.verifyQuoLegacySignature(PRETTY_BODY, signLegacy(PRETTY_BODY), TEST_LEGACY_SECRET)
    ).toBe(true);
  });
  ...
  it('handles non-ASCII bodies (multi-byte UTF-8) in both schemes', () => {
    ...
    expect(route.verifyQuoLegacySignature(body, signLegacy(body), TEST_LEGACY_SECRET)).toBe(true);
    expect(route.verifyQuoSignature(body, signCurrent(body), TEST_WEBHOOK_KEY)).toBe(true);
  });
});
