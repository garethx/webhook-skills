...
over `<timestamp>.<raw_body>` keyed by the client secret. Deliveries older than 48 hours are rejected.
When that header is absent it falls back to the **legacy** `X-Recharge-Hmac-Sha256` header — which,
despite the name, is a **plain SHA-256** of `client_secret + raw_body` (secret first), hex-encoded —
**not HMAC**. The handler reads the raw body with `await request.body()` before parsing so the exact
bytes are hashed.
