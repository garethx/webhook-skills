# Utila Webhooks - Express Example

Minimal example of receiving Utila webhooks with RSA signature verification.
...
- `express.raw()` captures the exact request bytes — the RSA signature is
  computed over the raw body, so it must not be parsed to JSON first.
