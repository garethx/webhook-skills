# Smile API Webhooks - Next.js Example

Minimal example of receiving Smile API (getsmileapi.com) webhooks in a Next.js
App Router route handler with `Smile-Signature` (HMAC-SHA512) verification.
...
3. Set `SMILE_WEBHOOK_SECRET` in `.env.local` to the secret you set when
...
The webhook route is `POST /webhooks/smile` (`app/webhooks/smile/route.ts`),
