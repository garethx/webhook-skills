 ...
 */
export async function verifyMondayJwt(authHeader, secret) {
...
// monday.com webhook endpoint. JSON body parsing is fine here because the JWT
// signature does not cover the body (unlike Stripe/GitHub HMAC schemes).
app.post('/webhooks/monday', express.json(), async (req, res) => {
  ...
  if (process.env.MONDAY_SIGNING_SECRET) {
    try {
      await verifyMondayJwt(req.headers['authorization'], process.env.MONDAY_SIGNING_SECRET);
    } catch (err) {
...
if (isMainModule) {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`Webhook endpoint: POST http://localhost:${PORT}/webhooks/monday`);
  });
}
