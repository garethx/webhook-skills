...
const { app, authClient } = require('../src/index');

const AUDIENCE = 'https://example.com/webhooks/google-pubsub';
...
function createToken(overrides = {}, { kid = KID } = {}) {
...
/** Build a wrapped push envelope. `data` is base64; omit it for attribute-only. */
function pushEnvelope({ data = { orderId: '123', total: 4995 }, attributes, ...rest } = {}) {
  const message = {
  ...
  return { message, subscription: SUBSCRIPTION, ...rest };
}

const post = (body, token) => {
  const req = request(app).post('/webhooks/google-pubsub').set('Content-Type', 'application/json');
...
describe('OIDC token verification', () => {
  ...
  it('rejects a request with no Authorization header', async () => {
    ...
    expect(res.text).toBe('Invalid OIDC token');
  });

  it('rejects a non-Bearer Authorization scheme', async () => {
    const res = await request(app)
      .post('/webhooks/google-pubsub')
      .set('Authorization', `Basic ${createToken()}`)
      .send(pushEnvelope());
    expect(res.status).toBe(401);
  });
  ...
  it('rejects a token from a different service account', async () => {
    ...
    const res = await post(pushEnvelope(), createToken({ email: 'other@other.iam.gserviceaccount.com' }));
    expect(res.status).toBe(401);
  });
  ...
  it('rejects a token whose payload has been swapped (signature no longer matches)', async () => {
    const token = createToken();
    const [header, , signature] = token.split('.');
    const forged = b64url({ iss: 'https://accounts.google.com', aud: AUDIENCE, email: SERVICE_ACCOUNT });
    const res = await post(pushEnvelope(), `${header}.${forged}.${signature}`);
    expect(res.status).toBe(401);
  });

  it('rejects a token signed with an unknown key id', async () => {
    const res = await post(pushEnvelope(), createToken({}, { kid: 'unknown-kid' }));
    expect(res.status).toBe(401);
  });
});
...
describe('push envelope handling', () => {
  ...
  it('rejects an envelope with no message object', async () => {
    ...
    expect(res.text).toBe('Invalid Pub/Sub push envelope');
  });
  ...
  it('authenticates before parsing the envelope', async () => {
    ...
    const res = await post({ nonsense: true }, createToken({ aud: 'https://wrong.example.com' }));
    expect(res.status).toBe(401);
  });
});
...
describe('subscription allowlist', () => {
  ...
  it('rejects a push from an unexpected subscription', async () => {
    ...
    expect(res.text).toBe('Untrusted subscription');
  });
});
...
describe('URL verification token (unauthenticated subscriptions)', () => {
  ...
  it('acks a push carrying the correct token', async () => {
    const res = await request(app)
      .post('/webhooks/google-pubsub?token=a-long-unguessable-token')
      .send(pushEnvelope());
    expect(res.status).toBe(204);
  });
  ...
  it('rejects a push carrying the wrong token', async () => {
    const res = await request(app)
      .post('/webhooks/google-pubsub?token=wrong')
      .send(pushEnvelope());
    expect(res.status).toBe(401);
    expect(res.text).toBe('Invalid verification token');
  });
...
describe('fail-closed configuration', () => {
  ...
  it('refuses every request when no authentication is configured', async () => {
    ...
    expect(res.text).toMatch(/not configured to authenticate/);
  });
