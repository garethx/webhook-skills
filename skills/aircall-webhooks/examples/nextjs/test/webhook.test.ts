...
// Test token — set before importing the route.
...
import { POST, GET, verifyAircallWebhook } from '../app/webhooks/aircall/route';
...
function createRequest(
  ...
  return new NextRequest('http://localhost:3000/webhooks/aircall', {
    method: 'POST',
    headers: {
...
describe('Aircall Webhook Handler (Next.js)', () => {
  describe('GET /webhooks/aircall', () => {
  ...
  describe('POST /webhooks/aircall — token verification', () => {
    it('rejects a payload with no token field', async () => {
      const payload: Record<string, any> = buildPayload('call.created', CALL_DATA);
    ...
    it('rejects a payload with the wrong token', async () => {
      const response = await POST(
        createRequest(buildPayload('call.created', CALL_DATA, { token: 'wrong_token_value' }))
      );
    ...
    it('rejects a token that differs only in the last character', async () => {
      ...
      const response = await POST(
        createRequest(buildPayload('call.created', CALL_DATA, { token: nearMiss }))
      );
    ...
    it('rejects a different-length token (timing-safe path)', async () => {
      const response = await POST(
        createRequest(buildPayload('call.created', CALL_DATA, { token: 'short' }))
      );
    ...
    it('accepts a payload with the correct token', async () => {
      const response = await POST(createRequest(buildPayload('call.created', CALL_DATA)));

      expect(response.status).toBe(200);
      expect(await response.json()).toEqual({ received: true, event: 'call.created' });
    });
  });

  describe('POST /webhooks/aircall — payload validation', () => {
    it('returns 400 for invalid JSON', async () => {
      const response = await POST(createRequest(null, { rawBody: '{ not valid json' }));
    ...
    it('returns 400 when the event field is missing', async () => {
      ...
      expect(response.status).toBe(400);
      expect(await response.text()).toBe('Missing event');
    });
  ...
  describe('POST /webhooks/aircall — call events', () => {
    ...
    it.each(callEvents)('handles %s', async (event) => {
      const response = await POST(
        createRequest(buildPayload(event, { ...CALL_DATA, tags: [{ name: 'support' }] }))
      );

      expect(response.status).toBe(200);
      expect(await response.json()).toEqual({ received: true, event });
    });
    ...
    it('handles call.ended with duration and cost', async () => {
      ...
      expect(response.status).toBe(200);
      expect((await response.json()).event).toBe('call.ended');
    });
  });
  ...
  describe('POST /webhooks/aircall — user events', () => {
    ...
    it.each(userV1Events)('still handles deprecated %s', async (event) => {
      const response = await POST(
        createRequest(
          buildPayload(event, { id: 456, name: 'Madelaine Dupont' }, { resource: 'user' })
        )
      );
  ...
  describe('POST /webhooks/aircall — number events', () => {
    it.each(['number.created', 'number.opened', 'number.closed', 'number.deleted'])(
      ...
      async (event) => {
        const response = await POST(
          createRequest(buildPayload(event, NUMBER_DATA, { resource: 'number' }))
        );
    ...
    it('handles the documented number.closed payload verbatim', async () => {
      ...
      expect(response.status).toBe(200);
      expect(await response.json()).toEqual({ received: true, event: 'number.closed' });
    });
  });
  ...
  describe('POST /webhooks/aircall — message events', () => {
    ...
    it('handles a WhatsApp message', async () => {
      ...
      expect(response.status).toBe(200);
      expect((await response.json()).event).toBe('message.received');
    });
  ...
  describe('POST /webhooks/aircall — conversation intelligence & AI events', () => {
    ...
    it.each([
      ...
      const response = await POST(
        createRequest(buildPayload(event, { call_id: 123 }, { resource: 'ai_voice_agent' }))
      );
    ...
    it('handles integration.deleted', async () => {
      ...
      expect(response.status).toBe(200);
      expect(await response.json()).toEqual({ received: true, event: 'integration.deleted' });
    });
  });
  ...
  describe('POST /webhooks/aircall — forward compatibility & idempotency', () => {
    it('returns 200 for an unknown event rather than failing', async () => {
      ...
      const response = await POST(
        createRequest(buildPayload('call.some_future_event', CALL_DATA))
      );

      expect(response.status).toBe(200);
      expect(await response.json()).toEqual({
        received: true,
        event: 'call.some_future_event',
      });
    });

    it('returns 200 for an unknown resource type', async () => {
      const response = await POST(
        createRequest(buildPayload('future_resource.created', {}, { resource: 'future_resource' }))
      );
    ...
    it('accepts out-of-order deliveries (no ordering guarantee)', async () => {
      ...
      const ended = await POST(
        createRequest(buildPayload('call.ended', { ...CALL_DATA, duration: 60 }))
      );
      const created = await POST(createRequest(buildPayload('call.created', CALL_DATA)));
    ...
    it('accepts an old timestamp — there is no replay/staleness window', async () => {
      ...
      const response = await POST(
        createRequest(buildPayload('call.created', CALL_DATA, { timestamp: 1 }))
      );

      expect(response.status).toBe(200);
      expect((await response.json()).event).toBe('call.created');
    });
  });
});
