# Customer.io Webhooks - Next.js Example
...
The webhook route is `POST /webhooks/customerio` at http://localhost:3000.
...
The route reads the **raw** request body with `await request.text()` before parsing JSON, then
recomputes HMAC-SHA256 over `v0:<X-CIO-Timestamp>:<raw body>` and compares (timing-safe, hex)
against `X-CIO-Signature`. See [../../references/verification.md](../../references/verification.md).
