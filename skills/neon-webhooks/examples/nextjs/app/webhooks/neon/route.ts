...
async function getPublicKey(kid: string): Promise<crypto.KeyObject> {
  ...
  const jwks = (await res.json()) as { keys: crypto.JsonWebKey[] };

  const jwk = jwks.keys.find((k) => (k as { kid?: string }).kid === kid);
...
export async function POST(request: NextRequest) {
  ...
  const eventId = request.headers.get('x-neon-event-id');
  ...
  switch (eventType) {
    ...
    case 'user.before_create':
      ...
      // TODO: validate the signup; return a 400 to reject it.
      break;
