...
// Must be in the environment before the route handler reads it.
...
} from '../app/webhooks/mailersend/route';
...
function makeRequest(rawBody: string, signature?: string): NextRequest {
  ...
  if (signature !== undefined) headers.set('Signature', signature);
  return new NextRequest('https://example.com/webhooks/mailersend', {
    method: 'POST',
    headers,
    body: rawBody,
  });
...
function activityEvent(type = 'activity.sent', id = crypto.randomUUID()): string {
...
describe('signature verification', () => {
  ...
  it('rejects a signature computed with the wrong secret', async () => {
    ...
    const res = await POST(makeRequest(body, sign(body, 'the_wrong_secret')));
  ...
  it('rejects a body that is only re-serialised, not changed semantically', async () => {
    ...
    const res = await POST(makeRequest(reserialised, signature));
  ...
  it('returns 400 when the Signature header is missing', async () => {
    ...
    await expect(res.json()).resolves.toEqual({ error: 'Missing Signature header' });
  });
  ...
  it('does not throw on a non-hex signature of the right length', async () => {
    const body = activityEvent();
    const res = await POST(makeRequest(body, 'z'.repeat(64)));
  ...
  it('accepts an uppercase hex signature', async () => {
    const body = activityEvent();
    const res = await POST(makeRequest(body, sign(body).toUpperCase()));
  ...
  it('reads the Signature header case-insensitively', async () => {
    const body = activityEvent();
    const headers = new Headers({ 'content-type': 'application/json' });
    headers.set('signature', sign(body)); // lowercase on the wire
    const req = new NextRequest('https://example.com/webhooks/mailersend', {
      method: 'POST',
      headers,
      body,
    });
...
describe('webhook.test URL-validation ping', () => {
  it('accepts the ping signed with the fixed public test secret', async () => {
    ...
    const res = await POST(makeRequest(TEST_PING, sign(TEST_PING, MAILERSEND_TEST_SECRET)));
  ...
  it('also accepts the ping signed with the webhook signing secret', async () => {
    const res = await POST(makeRequest(TEST_PING, sign(TEST_PING, SECRET)));

    expect(res.status).toBe(200);
  });

  it('rejects the ping when signed with neither secret', async () => {
    const res = await POST(makeRequest(TEST_PING, sign(TEST_PING, 'some_other_secret')));
...
describe('the public test secret cannot authorise a real event', () => {
  it('rejects activity.hard_bounced signed with the public test secret', async () => {
    ...
    const res = await POST(makeRequest(body, sign(body, MAILERSEND_TEST_SECRET)));
...
describe('event handling', () => {
  ...
  it('is idempotent: the same data.id is only processed once', async () => {
    const body = activityEvent('activity.opened', 'dedupe-me-once');
    const signature = sign(body);
