# PayPro Global Webhooks - Express Example
...
3. Set `PAYPRO_VALIDATION_KEY` (for `SIGNATURE`) and, optionally,
   `PAYPRO_SECRET_KEY` (for `HASH`) in `.env`. These are **two different keys**.
...
2. **Verify `SIGNATURE`** (SHA256, primary) — recompute from the form field
   values and compare timing-safely (`400` on mismatch).
...
`express.urlencoded()`. The `SIGNATURE` covers specific field **values**, not the
