...
export async function POST(request: NextRequest) {
  // Read the RAW body and verify Upollo-Signature before parsing.
  const body = await request.text();
  const signature = request.headers.get('upollo-signature');

  if (!verifyUpolloWebhook(body, signature, process.env.UPOLLO_WEBHOOK_SECRET!)) {
