...
**Audience matters.** Declare the platform's intended audience at audit start: `developer-platform` (default, where integrators are software engineers) or `no-code-saas` (where integrators are power users wiring up automations through a UI). The audience-driven N/A logic in `rubric.md` removes criteria that don't apply to the audience (e.g. IaC and local-dev workflow simulation are N/A for a no-code SaaS). If unsure, default to `developer-platform` and surface the rubric gaps to be discussed with the platform.
