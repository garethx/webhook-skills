...
- **Content-based filtering.** Beyond choosing which event *types* to subscribe to, can the consumer filter deliveries on the *payload content* — so a destination only receives events whose fields match a condition (e.g. `amount > 1000`, `status == "paid"`, or a JSONPath/CEL/expression predicate on the body)? This is the Event Destinations initiative's recommended payload-filtering capability, distinct from event-type granularity above: type subscription decides which event names flow; content filtering decides which *instances* of those events flow based on their data. 0: no content/payload filtering — selection is by event type only. 1: limited or undocumented content filtering (e.g. a single fixed field, an attribute-only filter that does not reach into the payload body, or the capability exists but is thinly documented). 2: documented predicate/expression filtering on payload content, applied per destination/subscription. (Distinct from Cat 6 ordering & rate controls, which shape delivery flow, not which events qualify.)
