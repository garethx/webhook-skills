...
const SUBSCRIPTION_DELETED_EVENT_TYPE = 'Microsoft.EventGrid.SubscriptionDeletedEvent';
...
const DEFAULT_SECRET_HEADER = 'x-eventgrid-token';
...
const DEFAULT_SECRET_QUERY_PARAM = 'token';
...
function checkAgainstAny(received, expectedCsv) {
  ...
  return accepted.reduce((matched, candidate) => checkDeliverySecret(received, candidate) || matched, false);
}
 ...
 *
 * @returns the token claims, or null if validation failed.
 */
 ...
 *
 * @param getHeader  (name) => string | undefined, lowercase header lookup
 * @param isValidation  true when the body carries a SubscriptionValidationEvent
 */
async function authenticate(getHeader, { isValidation = false, getQueryParam = () => undefined } = {}) {
 ...
 * single JSON OBJECT.
 *
 * @returns an array of normalised events, or null if the body isn't usable.
 */
 ...
 */
function findValidationCode(events) {
  const validation = events.find((event) => event.type === VALIDATION_EVENT_TYPE);
  const code = validation && validation.data && validation.data.validationCode;
 ...
 */
function handleAbuseProtection(requestOrigin) {
