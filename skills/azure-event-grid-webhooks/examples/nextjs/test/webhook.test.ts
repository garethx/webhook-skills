...
import { OPTIONS, POST } from '../app/webhooks/azure-event-grid/route';
import { checkAgainstAny, checkDeliverySecret, signingKeys } from '../lib/eventgrid';

const WEBHOOK_URL = 'http://localhost:3000/webhooks/azure-event-grid';
...
function signToken(overrides: Record<string, any> = {}, key: any = privateKey) {
...
function post(
  ...
  }: { headers?: Record<string, string | null>; contentType?: string } = {}
) {
  ...
  for (const [name, value] of Object.entries(headers)) {
    if (value === null) delete merged[name.toLowerCase()];
    else merged[name.toLowerCase()] = value;
  }
  return POST(
...
beforeEach(() => {
  signingKeys.resolve = async (kid: string) => {
...
describe('subscription validation handshake (Event Grid schema)', () => {
  ...
  it('refuses to echo the code for an unrecognized subscription', async () => {
    ...
    expect((await res.json()).validationResponse).toBeUndefined();
  });
...
describe('CloudEvents v1.0 abuse-protection preflight (HTTP OPTIONS)', () => {
  const options = (headers: Record<string, string> = {}) =>
    OPTIONS(new Request(WEBHOOK_URL, { method: 'OPTIONS', headers }));
  ...
  it('grants consent by echoing the requested origin', async () => {
    ...
    const res = await options({ 'webhook-request-origin': 'eventemitter.example.com' });

    expect(res.status).toBe(200);
    expect(res.headers.get('webhook-allowed-origin')).toBe('eventemitter.example.com');
    expect(res.headers.get('webhook-allowed-rate')).toBe('120');
  ...
  it('honours a custom allowed rate', async () => {
    ...
    expect(res.headers.get('webhook-allowed-rate')).toBe('600');
  });

  it('withholds the grant headers for an origin that is not allowed', async () => {
    process.env.AZURE_EVENT_GRID_ALLOWED_ORIGINS = 'eventemitter.example.com';

    const res = await options({ 'webhook-request-origin': 'attacker.example.com' });

    // Consent is signalled by the headers, not the status code.
    expect(res.headers.get('webhook-allowed-origin')).toBeNull();
    expect(res.headers.get('webhook-allowed-rate')).toBeNull();
  });
 ...
 */
function postWithQuery(body: unknown, { query = '' }: { query?: string } = {}) {
  return POST(
    new Request(`${WEBHOOK_URL}${query}`, {
      method: 'POST',
      headers: {
...
describe('query-parameter channel authentication', () => {
  ...
  it('accepts BOTH old and new secrets during rotation', async () => {
    ...
    expect((await postWithQuery([blobCreatedEvent()], { query: `?token=${oldSecret}` })).status).toBe(200);
    expect((await postWithQuery([blobCreatedEvent()], { query: `?token=${newSecret}` })).status).toBe(200);
    expect(
      (await postWithQuery([blobCreatedEvent()], { query: `?token=${'z'.repeat(64)}` })).status
    ).toBe(401);
  });
  ...
  it('preserves secret case (the accepted list must not be lowercased)', async () => {
    ...
    expect((await postWithQuery([blobCreatedEvent()], { query: `?token=${mixedCase}` })).status).toBe(200);
    expect(
      (await postWithQuery([blobCreatedEvent()], { query: `?token=${mixedCase.toLowerCase()}` })).status
    ).toBe(401);
  });
...
describe('Microsoft Entra ID channel authentication', () => {
  ...
  it('rejects a token signed by a different key', async () => {
    ...
    const res = await withToken(signToken({}, other.privateKey));
    expect(res.status).toBe(401);
  });
  ...
  it('rejects an unsigned (alg: none) token', async () => {
    const payload = signToken().split('.')[1];
