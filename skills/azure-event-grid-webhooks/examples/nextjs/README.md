# Azure Event Grid Webhooks - Next.js Example

Minimal example of receiving Azure Event Grid deliveries with the Next.js App
Router: both endpoint-validation handshakes, the `aeg-subscription-name`
identity guard, and channel authentication via a static delivery-property header
or a Microsoft Entra ID bearer token.
...
Server runs on http://localhost:3000. The route is
`app/webhooks/azure-event-grid/route.ts`, exporting `POST` for deliveries and
`OPTIONS` for the CloudEvents preflight.
...
subscription is validated the moment you create it, so start the dev server first.
...
2. **`POST`** calls `request.json()` first. Nothing is signed, so there is no
   raw-body requirement — this is the one webhook family where parsing before
...
The route sets `runtime = 'nodejs'` because `jsonwebtoken` and `jwks-rsa` need
Node crypto, and `dynamic = 'force-dynamic'` so it is never statically
optimised.
...
The tests call the exported `POST` and `OPTIONS` handlers directly with real
`Request` objects. They cover both handshakes, the subscription-name guard
(including the refusal to echo a validation code for an unknown subscription),
Event Grid array vs CloudEvents object normalisation, batched arrays,
